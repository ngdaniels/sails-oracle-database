    ...
    var adapter = {
        ...
        find: function(connectionName, table, cb) {
            ...
            Object.keys(connectionObject.collections).forEach(function(coll) {
                schema[coll] = connectionObject.collections[coll];
            });
            ...
            var limit = _options.limit || null;
            var skip = _options.skip || null;
        ...
        create: function(connectionName, table, cb) {
            ...
            var connectionObject = connections[connectionName];
            var collection = connectionObject.collections[table.using];
            ...
            Object.keys(schema[table.using].definition).forEach(function(column) {
                //TODO: parse date to oracle format
                if (schema[table.using].definition[column].type === 'datetime') {
                    cb[column] = new Date(cb[column]); //'to_date('+moment(data[column]).format('YYYY/MM/DD HH:mm:ss')+','+formatDate+')';//moment(data[column]).format('YYYY/MM/DD HH:mm:ss');
                }
            });

            // Build a query for the specific query strategy
            try {
                query = sequel.create(table, cb);
            } catch (e) {
                return cb(e);
            }
            ...
            connectionObject.pool.getConnection(
                function(err, connection) {
                    ...
                    connection.execute(query.query, _.values(cb), { /*outFormat: oracledb.OBJECT, */
                        autoCommit: false
